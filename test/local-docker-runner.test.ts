...
describe("LocalDockerJobRunner", () => {
  ...
  it("runIssue does not emit duplicate --label flags", async () => {
    const { calls, spawnStub } = makeSpawnStub();
    const runner = new LocalDockerJobRunner(spawnStub);
    ...
    const args = calls[0].args as string[];
    const managedLabelCount = args.filter((a, i) => a === "--label" && args[i + 1] === "autoworker.managed=true").length;
    expect(managedLabelCount).toBe(1);
    const corrLabelCount = args.filter((a, i) => a === "--label" && args[i + 1] === "autoworker.correlationId=c1").length;
    expect(corrLabelCount).toBe(1);
  });

  it("runPrReview does not emit duplicate --label flags", async () => {
    const { calls, spawnStub } = makeSpawnStub();
    const runner = new LocalDockerJobRunner(spawnStub);
    ...
    const managedLabelCount = args.filter((a, i) => a === "--label" && args[i + 1] === "autoworker.managed=true").length;
    expect(managedLabelCount).toBe(1);
    const corrLabelCount = args.filter((a, i) => a === "--label" && args[i + 1] === "autoworker.correlationId=pr-c1").length;
    expect(corrLabelCount).toBe(1);
  });
